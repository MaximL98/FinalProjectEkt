...
public class InventoryRestockWorkerPageController {
	...
	void getComboMachine(ActionEvent event) {
		...
		if (inventoryList == null) {
			...
			selectedMachine.setProducts(getProductsForMachine(selectedMachine));
			for (ProductInMachine p : selectedMachine.getProducts()) {
	...
	@FXML
	void initialize() {
		ObservableList<Machine> machines = FXCollections.observableArrayList(getMachines(null));
		if (machines == null)
			return;
		...
		colRestockAmount.setOnEditCommit(event -> {
			...
			try {
				...
				// non-negative
				// set the value to set as the new value.
				// Otherwise we set back the old value.
	...
	private ArrayList<Machine> getMachines(Location[] locations) {
		...
		// otherwise we create a Machine arrayList and add the items from response
		// to it.
