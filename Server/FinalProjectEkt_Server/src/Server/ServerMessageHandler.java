...
public class ServerMessageHandler {
	...
	/**
	 * HandleMessageLogin is a private, final class that implements
	 * IServerSideFunction. It is responsible for handling login requests from
	 * clients. When a client sends a login request, the handleMessage method is
	 * called. This method first verifies that the request type is LOGIN and that
	 * the message sent is an array of two strings, the username and the password.
	 * Then, it checks if the user exists in the database and if the password is
	 * correct. If the user does not exist or the password is incorrect, the method
	 * sends a LOGIN_FAILED_ILLEGAL_INPUT response to the client. If the user exists
	 * and the password is correct, it checks if the user is already connected. If
	 * the user is already connected, it sends a LOGIN_FAILED_ALREADY_LOGGED_IN
	 * response to the client. If the user is not already connected, it connects the
	 * user by inserting them into the logged_users table in the database, and sends
	 * a LOGIN response to the client. If any errors occur during the login process,
	 * the method sends an ERROR_MESSAGE response to the client.
	 * 
	 * @author Rotem
	 *
	 */
	private static final class HandleMessageLogin implements IServerSideFunction {
		...
		public SCCP handleMessage(SCCP loginMessage) {
			...
			if (res == null) {
				System.out.println(
						"Server failed login request for user " + username + " (invalid username or password).");
			...
			if (res instanceof SystemUser) {
				...
				Object res2 = DatabaseController.handleQuery(DatabaseOperation.GENERIC_SELECT,
						new Object[] { sqlQueryLoggedInTest });
				...
				if ((!(actualResult2 == null)) && actualResult2.size() != 0) {
					System.out
							.println("Server login request for user " + username + " failed - user already connected.");
				...
				DatabaseController.handleQuery(DatabaseOperation.INSERT,
						new Object[] { "logged_users", false, new Object[] { "(\"" + username + "\")" } });
				System.out.println("Sent third login query!");
				System.out.println("Server login request for user " + username
						+ " completed: user inserted to logged_users table.");

				return new SCCP(ServerClientRequestTypes.LOGIN, (SystemUser) res);
			}
			// we know that something wrong occurred
			System.out.println(
					"Server login request for user " + username + " completed: user inserted to logged_users table.");
