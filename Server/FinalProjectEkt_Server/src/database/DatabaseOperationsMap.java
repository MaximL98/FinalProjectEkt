...
public class DatabaseOperationsMap {
	...
	protected static final class DatabaseActionSelectForFetchOrders implements IDatabaseAction {
		...
		public Object getDatabaseAction(Object[] orderFilters) {
			 ...
			 * TARGET SQL:
			 * 
			 * SELECT * from ektdb.orders JOIN ektdb.machine using(machineId) JOIN
			 * ektdb.order_type using(typeId) JOIN ektdb.order_status using(statusId) JOIN
			 * ektdb.locations using(locationId) (optional) WHERE statusName = "?"
			 * (optional)
			 * 
			 */
			...
			sqlBuilder.append(JOIN);
			sqlBuilder.append(ORDER_TYPES_TABLE);
			sqlBuilder.append("using(typeId) ");
			sqlBuilder.append(JOIN);
			sqlBuilder.append(ORDER_STATUSES_TABLE);
			sqlBuilder.append("using(statusId) ");
			sqlBuilder.append(JOIN);
			sqlBuilder.append(LOCATIONS_TABLE);
			sqlBuilder.append("using(locationId)");

			if (orderFilters.length > 0) {
				String statusName = ((String[]) orderFilters[0])[0];
				sqlBuilder.append(" WHERE statusName = \"");
				sqlBuilder.append(statusName);
				sqlBuilder.append("\"");
			}
			sqlBuilder.append(";");
			...
			try {
				while (fetchOrdersResultSet.next()) {
					...
					Location location = Location.valueOf(fetchOrdersResultSet.getString("locationName"));
					Machine machine = new Machine(machineId, machineName, location);
					
					Type type = Type.values()[fetchOrdersResultSet.getInt("typeId") - 1];
					Status status = Status.values()[fetchOrdersResultSet.getInt("statusId") - 1];
					LocalDate dateReceived = fetchOrdersResultSet.getDate("date_recieved").toLocalDate();
	...
	private static HashMap<DatabaseOperation, IDatabaseAction> map = new HashMap<DatabaseOperation, IDatabaseAction>() {
...

		{
			...
			this.put(DatabaseOperation.FETCH_ONLINE_ORDERS, new DatabaseActionSelectForFetchOrders());
			this.put(DatabaseOperation.UPDATE_ONLINE_ORDERS, new DatabaseActionUpdateForUpdateOrders());
